...
class MapReader {
    ...
    findInsertionPoint(records, newRecord) {
        for (let idx = 0; idx < records.length; idx++) {
            let record = records[idx];
            if (record.RCNM == newRecord.RCNM && record.RCID > newRecord.RCID) {
                return idx - 1;
            }
            if (newRecord.RCNM == 130 && record.RCNM == 100) {
                return idx;
            }
            if (newRecord.RCNM > 100 && record.RCNM > 100 && newRecord.RCNM < record.RCNM) {
                return idx;
            }
        }
        return null;
    ...
    updateRecords(records, loadedRecords) {
        for (let record of loadedRecords) {
            ...
            if (action == 1) {
                this.opts.debug && console.log(`Inserting RCNM: ${record.RCNM}, RCID:  ${record.RCID}`);
                if (result) {
                    records[result.Index] = record;
                } else {
                    let position = this.findInsertionPoint(records, record);
                    if (position != null) {
                ...
                this.opts.debug && console.log(`Deleting RCNM: ${record.RCNM}, RCID:  ${record.RCID}`);
                if (result) {

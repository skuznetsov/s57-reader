        ...
        if (dsid) {
            ...
            if (attf) {
                for (let idx = 0; idx < attf.GroupsCount; idx++) {
                    ...
                    if (attr) {
                        feature.Attributes[attr.Code] = {Name: attr.Code, Description: attr.Description, Value: attributeValue};
                    }
                }
            }
    ...
    readFeatureSegments(record, feature, subFieldName) {
        ...
        if (subField) {
            ...
            for (let idx = 0; idx < subField.GroupsCount; idx++) {
                ...
                let rc = this._nameFromRawName(rawName);
                let obj = this.Nodes[id];
                if (!obj) {
                    console.log(`Node ${id} not found`);
                }
                child.ID = id;
                child.Type = rawName[0];
                child.RCID = rc.RCID;
            ...
            // if (children.length > 0 && children[0].Type > 120) {
            //     children.sort((a, b) => a.Type * a.RCID < b.Type * b.RCID);
            // }
